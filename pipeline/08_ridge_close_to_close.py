...
Target:  ctc_resid_t = (Close_{t+1}/Close_t - 1) - beta * SPY_ctc_t
         i.e. beta-residualized close-to-close return starting that same close.

No prediction lag needed: X_t and Y_t are aligned on the same date
(OFI through close of t predicts what happens from that close to next close).

Close-to-close returns are downloaded via yfinance and cached to
    data/processed/close_to_close_returns_residualized.csv

Run from repo root:
    python pipeline/08_ridge_close_to_close.py
"""
...
CTC_CACHE   = Path("data/processed/close_to_close_returns_residualized.csv")
OUT_DIR     = Path("results/rolling_adjacency_ridge_ctc")
...
REFIT_EVERY_DAYS   = 21
BETA_WINDOW        = 60   # days for rolling SPY beta on ctc returns

RIDGE_GRID = [1.0, 10.0, 50.0, 250.0, 1000.0]

MODEL_SPECS = [
    # ── baselines ───────────────────────────────────────────────
    {"name": "ridge_ctc_sb",             "structure": "sector_block", "use_gd": False,
     "hl": 45, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd",          "structure": "sector_block", "use_gd": True,
     "hl": 45, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    # ── half-life sweep ──────────────────────────────────────────
    {"name": "ridge_ctc_sb_gd_hl15",     "structure": "sector_block", "use_gd": True,
     "hl": 15, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl20",     "structure": "sector_block", "use_gd": True,
     "hl": 20, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl25",     "structure": "sector_block", "use_gd": True,
     "hl": 25, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl30",     "structure": "sector_block", "use_gd": True,
     "hl": 30, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl35",     "structure": "sector_block", "use_gd": True,
     "hl": 35, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl40",     "structure": "sector_block", "use_gd": True,
     "hl": 40, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl60",     "structure": "sector_block", "use_gd": True,
     "hl": 60, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl90",     "structure": "sector_block", "use_gd": True,
     "hl": 90, "sig_cols": None, "rank_norm": False, "zero_diag": False},
    # ── rank-normalized OFI ──────────────────────────────────────
    {"name": "ridge_ctc_sb_gd_hl30_rn",  "structure": "sector_block", "use_gd": True,
     "hl": 30, "sig_cols": None, "rank_norm": True,  "zero_diag": False},
    {"name": "ridge_ctc_sb_gd_hl90_rn",  "structure": "sector_block", "use_gd": True,
     "hl": 90, "sig_cols": None, "rank_norm": True,  "zero_diag": False},
    # ── multi-scale ─────────────────────────────────────────────
    {"name": "ridge_ctc_sb_gd_ms3090",   "structure": "sector_block", "use_gd": True,
     "hl": 30, "sig_cols": ["ofi_hl30", "ofi_hl90"], "rank_norm": False, "zero_diag": False},
    # ── multi-lag ────────────────────────────────────────────────
    {"name": "ridge_ctc_sb_gd_lag1",     "structure": "sector_block", "use_gd": True,
     "hl": 30, "sig_cols": ["ofi_hl30", "ofi_hl30_lag1"], "rank_norm": False, "zero_diag": False},
]


# ============================================================
# Gavish-Donoho optimal hard thresholding (unchanged from 27/31)
# ============================================================
...
# ============================================================
# close-to-close return computation
# ============================================================
def build_ctc_returns(tickers: list[str], cache_path: Path) -> pd.DataFrame:
    """Download daily OHLC, compute close-to-close returns residualized vs SPY.

    Returns wide DataFrame: index=date (the signal date, i.e. the starting close date),
    columns=tickers, values=residualized close-to-close return.

    ctc_ret[t] = (Close_{t+1}/Close_t - 1) - beta_t * SPY_ctc[t]
    """
    if cache_path.exists():
        print(f"Loading cached close-to-close returns from {cache_path}", flush=True)
    ...
    # close-to-close return for date t = close_{t+1} / close_t - 1
    # shift close by -1 so it aligns with the signal date (close date t)
    ctc = close_df.shift(-1) / close_df - 1.0
    # drop last row (no t+1 close available)
    ctc = ctc.iloc[:-1]

    spy_ctc = ctc["SPY"].copy()
    ctc = ctc.drop(columns=["SPY"], errors="ignore")

    # residualize each ticker's ctc return against SPY ctc
    print(f"Residualizing close-to-close returns (rolling {BETA_WINDOW}-day beta)...", flush=True)
    ctc_resid = ctc.copy() * np.nan
    for ticker in ctc.columns:
        s = ctc[ticker].dropna()
        combined = pd.concat([s, spy_ctc.reindex(s.index)], axis=1).dropna()
        ...
        ctc_resid.loc[resid.index, ticker] = resid.values

    ctc_resid.index.name = "date"
    ctc_resid.to_csv(cache_path)
...
# ============================================================
# load / build close-to-close returns and merge into df
# ============================================================
tickers_in_feature = df["ticker"].unique().tolist()
ctc_wide = build_ctc_returns(tickers_in_feature, CTC_CACHE)

# melt to long format and merge
ctc_long = (
    ctc_wide.reset_index()
    .melt(id_vars="date", var_name="ticker", value_name="ctc_resid")
    .dropna(subset=["ctc_resid"])
)
ctc_long["date"] = pd.to_datetime(ctc_long["date"])

df = pd.merge(df, ctc_long, on=["date", "ticker"], how="inner")
df = df.dropna(subset=["ctc_resid"]).copy()
print(f"  {len(df):,} rows after close-to-close merge", flush=True)
...
ret_proxy = df.pivot(index="date", columns="ticker", values="ctc_resid")
...
for model_spec in MODEL_SPECS:
    ...
    for block_num, start_offset in enumerate(refit_points, start=1):
        ...
        # ── close-to-close returns matrix ────────────────────────
        ret_mat = df_win.pivot(index="date", columns="ticker", values="ctc_resid").sort_index()
        ...
        # ── CTC ALIGNMENT: same date (no lag needed) ─────────────
        # X_t = today's OFI (through close of t)
        # Y_t = ctc_resid_t = close_t → close_{t+1}
        # Signal and return are registered to the same calendar date.
    ...
    ret_full = df.pivot(index="date", columns="ticker", values="ctc_resid").sort_index()
...
if all_model_summaries:
    ...
    print("\n\n" + "=" * 80)
    print("FINAL SUMMARY — close-to-close return prediction")
    print("=" * 80)

...
LAMBDA_GRID = [25.0, 50.0, 100.0, 250.0, 500.0, 1000.0, 2500.0]
...
# rolling settings
INITIAL_TRAIN_DAYS = 750     # about 3 years
REFIT_EVERY_DAYS = 21        # monthly refit

# PCA settings
N_PCA_COMPONENTS = 1

# Diagonal blend: add back alpha * diagonal to the off-diagonal W
# 0.0 = pure offdiag (original), 0.3 = blend in 30% of the auto-signal
DIAGONAL_BLEND_ALPHA = 0.3

# Vol scaling: scale each stock's position by 1/rolling_vol before computing spread
# Set to 0 to disable, otherwise use this many days for rolling vol estimate
VOL_SCALE_DAYS = 30
...
def daily_spread_volscaled(
    scores: pd.Series,
    returns: pd.Series,
    vol: pd.Series,
    q: float = 0.10,
) -> float:
    """Like daily_spread but weights positions by 1/vol before sorting."""
    tmp = pd.concat(
    ...
    # vol-adjust the score so that high-vol stocks are down-weighted
    tmp["score_adj"] = tmp["score"] / tmp["vol"].clip(lower=1e-8)
    tmp = tmp.sort_values("score_adj")

    # vol-weighted return: sum(ret_i / vol_i) / sum(1/vol_i)
...
def fit_model(
    ...
    if model_name == "sector_offdiag":
        ...
        W_offdiag = build_sector_offdiag(W_sector)
        if DIAGONAL_BLEND_ALPHA > 0.0:
            # Blend back a fraction of the diagonal (auto-signal)
            diag_vals = np.diag(W_sector.to_numpy())
            W_blended = W_offdiag.to_numpy().copy()
            np.fill_diagonal(W_blended, DIAGONAL_BLEND_ALPHA * diag_vals)
            return pd.DataFrame(W_blended, index=W_offdiag.index, columns=W_offdiag.columns)
        return W_offdiag
...
def evaluate_predictions(
    ...
    # Vol-scaled spread
    if vol_scale_days > 0:
        rolling_vol = ret_eval_df.rolling(vol_scale_days, min_periods=5).std()
        daily_spreads_volscaled = pred_df.apply(
            lambda row: daily_spread_volscaled(
                row,
                ret_eval_df.loc[row.name],
                vol=rolling_vol.loc[row.name].dropna(),
                q=q,
            ),
            axis=1
        ).dropna()
    else:
        daily_spreads_volscaled = daily_spreads.copy()

    return daily_spreads, daily_spreads_sector_neutral, daily_ic, daily_spreads_volscaled
...
def choose_lambda_on_window(
    ...
    """
    Holdout lambda selection: fit on all-but-last-21-days, score on last 21 days.
    This mirrors the actual prediction task (predict 21 days) and avoids in-sample
    overfitting that was pinning lambda to the minimum grid value.
    """
    n = len(X_train_z_df)

    # Need enough data to have a meaningful fit window
    if n <= holdout_days + 30:
        # Fallback to middle of grid if not enough data
        return float(lambda_grid[len(lambda_grid) // 2])

    X_fit = X_train_z_df.iloc[:-holdout_days]
    Y_fit = Y_train_z_df.iloc[:-holdout_days]
    X_val = X_train_z_df.iloc[-holdout_days:]
    Y_val = Y_train_z_df.iloc[-holdout_days:]
    ...
    for lam in lambda_grid:
        ...
        if isinstance(W, pd.DataFrame):
            ...
            pred = X_val.to_numpy(dtype=float) @ W

        # Score on holdout IC (more stable than Sharpe with only 21 days)
        pred_df = pd.DataFrame(pred, index=X_val.index, columns=X_val.columns)

        ic = pred_df.apply(
            lambda row: row.corr(Y_val.loc[row.name], method="spearman"),
            axis=1
        ).dropna()
...
daily_spreads, daily_spreads_sector_neutral, daily_ic, daily_spreads_volscaled = evaluate_predictions(
    pred_df=pred_all_df,
    ret_df=ret_eval_df,
    ticker_to_sector=ticker_to_sector_eval,
    q=Q,
    min_names_per_sector_neutral=MIN_NAMES_PER_SECTOR_NEUTRAL,
    vol_scale_days=VOL_SCALE_DAYS,
)
...
summary = pd.DataFrame([{
    ...
    "diagonal_blend_alpha": DIAGONAL_BLEND_ALPHA,
    "vol_scale_days": VOL_SCALE_DAYS,
    ...
    "mean_daily_spread_volscaled": daily_spreads_volscaled.mean(),
    "std_daily_spread_volscaled": daily_spreads_volscaled.std(ddof=1),
    "daily_spread_sharpe_volscaled": sharpe_from_series(daily_spreads_volscaled),
    "annualized_spread_sharpe_volscaled": annualize_sharpe(sharpe_from_series(daily_spreads_volscaled)),
...
daily_spreads_volscaled.to_csv(
    OUT_DIR / f"daily_spreads_volscaled_{MODEL_NAME}_rolling.csv",
    header=["spread_volscaled"]
)

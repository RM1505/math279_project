...
USE_CONTINUOUS_WEIGHTS = True
USE_SECTOR_NEUTRAL_CONTINUOUS = True
...
# ============================================================
# new portfolio-construction helpers
# ============================================================
...
def compute_trailing_vol_matrix(
    ret_df: pd.DataFrame,
    lookback: int,
    min_periods: int,
    floor: float,
) -> pd.DataFrame:
    vol = ret_df.shift(1).rolling(lookback, min_periods=min_periods).std()
    return vol.clip(lower=floor)


def compute_cross_sectional_rank_weights(
    score_row: pd.Series,
    sector_map: pd.Series | None = None,
    winsor_q: float = 0.01,
) -> pd.Series:
    score_row = score_row.replace([np.inf, -np.inf], np.nan)
    score_row = winsorize_series(score_row, q=winsor_q)
    ...
    if sector_map is None:
        ...
        ranks = valid.rank(method="average", pct=True) - 0.5
        denom = ranks.abs().sum()
        if denom > 0:
            weights.loc[valid.index] = ranks / denom
        return weights

    tmp = pd.concat(
        [score_row.rename("score"), sector_map.rename("sector")],
        axis=1
    ).dropna()
    ...
    for sector, g in tmp.groupby("sector"):
        ...
        ranks = g["score"].rank(method="average", pct=True) - 0.5
        denom = ranks.abs().sum()
        if denom <= 0:
            continue
    ...
    for w in sector_weights:
        weights.loc[w.index] = w / n_sectors
...
def evaluate_predictions(
    ...
    signal_smooth_days: int,
    vol_lookback_days: int,
    vol_min_periods: int,
    clip_vol_floor: float,
    score_winsor_q: float,
    use_vol_scaling: bool,
    use_signal_smoothing: bool,
    use_sector_neutral_continuous: bool,
):
    # align
    common_dates = pred_df.index.intersection(ret_df.index)
    common_cols = pred_df.columns.intersection(ret_df.columns)

    pred_df = pred_df.loc[common_dates, common_cols].copy()
    ret_df = ret_df.loc[common_dates, common_cols].copy()
    sector_map_eval = ticker_to_sector.loc[common_cols].copy()

    # -------------------------
    # old decile spread metrics
    # -------------------------
    daily_spreads = pred_df.apply(
        lambda row: daily_spread(row, ret_df.loc[row.name], q=q),
        axis=1
    ).dropna()
    ...
    daily_ic = pred_df.apply(
        lambda row: row.corr(ret_df.loc[row.name], method="spearman"),
        axis=1
    ).dropna()
    ...
    # -------------------------
    signal_df = pred_df.copy()

    if use_signal_smoothing and signal_smooth_days > 1:
        signal_df = signal_df.rolling(signal_smooth_days, min_periods=1).mean()

    vol_df = compute_trailing_vol_matrix(
        ret_df=ret_df,
        lookback=vol_lookback_days,
        min_periods=vol_min_periods,
        floor=clip_vol_floor,
    )

    if use_vol_scaling:
        score_df = signal_df / vol_df
    else:
        score_df = signal_df.copy()

    portfolio_weights = []
    portfolio_weights_sector_neutral = []

    for dt in score_df.index:
        score_row = score_df.loc[dt]

        w_all = compute_cross_sectional_rank_weights(
            score_row=score_row,
            sector_map=None,
            winsor_q=score_winsor_q,
        )
        portfolio_weights.append(w_all.rename(dt))

        w_sn = compute_cross_sectional_rank_weights(
            score_row=score_row,
            sector_map=sector_map_eval if use_sector_neutral_continuous else None,
            winsor_q=score_winsor_q,
        )
        portfolio_weights_sector_neutral.append(w_sn.rename(dt))

    weights_df = pd.DataFrame(portfolio_weights)
    weights_sector_neutral_df = pd.DataFrame(portfolio_weights_sector_neutral)

    portfolio_returns = (weights_df * ret_df).sum(axis=1).dropna()
    portfolio_returns_sector_neutral = (weights_sector_neutral_df * ret_df).sum(axis=1).dropna()
    ...
    return {
        ...
        "portfolio_returns_sector_neutral": portfolio_returns_sector_neutral,
        "weights_df": weights_df,
        "weights_sector_neutral_df": weights_sector_neutral_df,
        "score_df": score_df,
        "signal_df": signal_df,
        "vol_df": vol_df,
    }
...
for block_num, start_offset in enumerate(refit_points, start=1):
    ...
    if len(eligible_train_dates) < INITIAL_TRAIN_DAYS:
...
eval_results = evaluate_predictions(
    ...
    signal_smooth_days=SIGNAL_SMOOTH_DAYS,
    vol_lookback_days=VOL_LOOKBACK_DAYS,
    vol_min_periods=VOL_MIN_PERIODS,
    clip_vol_floor=CLIP_VOL_FLOOR,
    score_winsor_q=SCORE_WINSOR_Q,
    use_vol_scaling=USE_VOL_SCALING,
    use_signal_smoothing=USE_SIGNAL_SMOOTHING,
    use_sector_neutral_continuous=USE_SECTOR_NEUTRAL_CONTINUOUS,
)

daily_spreads = eval_results["daily_spreads"]
daily_spreads_sector_neutral = eval_results["daily_spreads_sector_neutral"]
daily_ic = eval_results["daily_ic"]
portfolio_returns = eval_results["portfolio_returns"]
portfolio_returns_sector_neutral = eval_results["portfolio_returns_sector_neutral"]
...
summary = pd.DataFrame([{
    ...
    "std_daily_portfolio_return": portfolio_returns.std(ddof=1),
    "daily_portfolio_sharpe": sharpe_from_series(portfolio_returns),
    "annualized_portfolio_sharpe": annualize_sharpe(sharpe_from_series(portfolio_returns)),

    "mean_daily_portfolio_return_sector_neutral": portfolio_returns_sector_neutral.mean(),
    "std_daily_portfolio_return_sector_neutral": portfolio_returns_sector_neutral.std(ddof=1),
    "daily_portfolio_sharpe_sector_neutral": sharpe_from_series(portfolio_returns_sector_neutral),
    "annualized_portfolio_sharpe_sector_neutral": annualize_sharpe(sharpe_from_series(portfolio_returns_sector_neutral)),
...
portfolio_returns.to_csv(
    OUT_DIR / f"portfolio_returns_{MODEL_NAME}_rolling.csv",
    header=["portfolio_return"]
)
portfolio_returns_sector_neutral.to_csv(
    OUT_DIR / f"portfolio_returns_sector_neutral_{MODEL_NAME}_rolling.csv",
    header=["portfolio_return_sector_neutral"]
)

eval_results["weights_df"].to_csv(OUT_DIR / f"portfolio_weights_{MODEL_NAME}_rolling.csv")
eval_results["weights_sector_neutral_df"].to_csv(
    OUT_DIR / f"portfolio_weights_sector_neutral_{MODEL_NAME}_rolling.csv"
)
eval_results["score_df"].to_csv(OUT_DIR / f"score_matrix_{MODEL_NAME}_rolling.csv")
eval_results["signal_df"].to_csv(OUT_DIR / f"signal_matrix_smoothed_{MODEL_NAME}_rolling.csv")
eval_results["vol_df"].to_csv(OUT_DIR / f"vol_matrix_{MODEL_NAME}_rolling.csv")

#!/usr/bin/env python3
"""31_ridge_overnight.py

Same rolling ridge framework as 27, but predicts OVERNIGHT returns
(close-to-open gap) instead of residualized intraday OPCL.
...
Overnight returns are downloaded via yfinance and cached to
    data/processed/overnight_returns_residualized.csv

Run from repo root:
    python cont_cucuringu_zhang/31_ridge_overnight.py
"""
...
OUT_DIR          = Path("results/rolling_adjacency_ridge_overnight")
...
def gavish_donoho_denoise(M: np.ndarray) -> tuple[np.ndarray, int, float]:
    ...
    try:
        ...
        # SVD failed to converge — return undenoised matrix
        return M.copy(), 0, 0.0
...
def build_overnight_returns(tickers: list[str], cache_path: Path) -> pd.DataFrame:
    ...
    # shift open by -1 so it aligns with the signal date (close date)
    overnight = open_df.shift(-1) / close_df - 1.0
    # drop last row (no t+1 open available)
    overnight = overnight.iloc[:-1]

    spy_on = overnight["SPY"].copy()
    overnight = overnight.drop(columns=["SPY"], errors="ignore")

    # residualize each ticker's overnight return against SPY overnight
...
if all_model_summaries:
    ...
    print("\n\n" + "=" * 80)
    print("FINAL SUMMARY — overnight return prediction")
    print("=" * 80)
